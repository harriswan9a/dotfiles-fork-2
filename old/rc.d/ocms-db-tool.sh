...
domain+=("dbr.pnx555.net") site+=("pnx") tz+=("+07:00")
domain+=("dbr.ezyserv1.com") site+=("ezb") tz+=("+07:00")
# INR
domain+=("dbr.jeetwin.com") site+=("jw") tz+=("+08:00") # 早期建置用 +0800
domain+=("dbr.win7699.com") site+=("gb") tz+=("+05:30")
# domain+=("dbr.goaadmin.com") site+=("gw") tz+=("+05:30") # 關站
domain+=("dbr.betpal247.com") site+=("bp") tz+=("+05:30")
# VND
...
for x in $(seq 0 ${endIndex}); do
  ...
  result=$(mysql -N -A -h${domain[x]} -u${user[x]} -D${db[x]} -t -e"${tzSql}${sql}")
  echo "${site[x]}\n${result}"
done
